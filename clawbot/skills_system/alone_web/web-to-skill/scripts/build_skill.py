...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--expected-name", help="Require metadata.name to match an existing skill during updates.")
...
def load_metadata(path: Path, page_urls: set[str], expected_name: str = "") -> dict[str, Any]:
    try:
    ...
    if expected_name and name != expected_name:
        raise ValueError(
            "metadata.name must remain unchanged during update: "
            f"expected {expected_name!r}, got {name!r}; "
            "set metadata.name to the exact expected value and rerun only the build stage"
        )
...
def load_crawl_coverage(path: Path, page_count: int) -> dict[str, int | bool]:
    ...
    reused_match = re.search(r"(?:^|\s)reused=(-?\d+)(?:\s|$)", run_done)
    coverage["reused"] = int(reused_match.group(1)) if reused_match else 0
    error_page_match = re.search(r"(?:^|\s)skipped_error_pages=(-?\d+)(?:\s|$)", run_done)
    coverage["skipped_error_pages"] = int(error_page_match.group(1)) if error_page_match else 0
    ...
    reused = int(coverage["reused"])
    duplicate_final_urls = int(coverage["duplicate_final_urls"])
    skipped_error_pages = int(coverage["skipped_error_pages"])
    ...
    if (
        ...
        or skipped_error_pages < 0
        or skipped < 0
    ):
        raise ValueError(f"crawl run.done has invalid negative or empty counts: {path}")
    if reused + attempted + skipped != requested:
        raise ValueError("crawl coverage mismatch: reused plus attempted plus timeout-skipped must equal requested")
    if succeeded + failed + duplicate_final_urls + skipped_error_pages != attempted:
        raise ValueError(
            "crawl coverage mismatch: fetched outcomes must equal attempted"
        )
    if reused + succeeded != page_count:
        raise ValueError("crawl coverage mismatch: reused plus succeeded must equal the number of index records")
    return coverage
...
def crawl_coverage_note(coverage: dict[str, int | bool]) -> str:
    ...
    skipped_error_pages = int(coverage["skipped_error_pages"])
    skipped = int(coverage["skipped_pages_after_timeout_stop"])
    captured = reused + succeeded
    reuse_note = f" Reused {reused} page{'s' if reused != 1 else ''} from the previous skill." if reused else ""
    error_page_note = (
        f" Skipped {skipped_error_pages} Yuque error page{'s' if skipped_error_pages != 1 else ''}."
        if skipped_error_pages
        else ""
    )
    if failed == 0 and skipped == 0 and duplicate_final_urls == 0 and skipped_error_pages == 0:
        if requested == 1:
            return "The prepared page was captured successfully." + reuse_note
        return f"All {requested} prepared pages were captured successfully." + reuse_note
    duplicate_note = (
    ...
    if failed == 0 and skipped == 0:
        return (
            f"Captured {captured} unique pages from {requested} prepared URLs."
            f"{reuse_note}{duplicate_note}{error_page_note}"
        )
    ...
    return (
        f"Crawl coverage is partial: captured {captured} of {requested} prepared {requested_label}; "
        f"{failed} attempted {failed_label} failed and {skipped} {skipped_label} skipped."
        f"{reuse_note}{duplicate_note}{error_page_note}{reason}"
    )
...
def build(args: argparse.Namespace) -> Path:
    ...
    metadata = load_metadata(metadata_path, {str(page["url"]) for page in pages}, str(args.expected_name or "").strip())
