...
<template>
  <a-modal
    width="700px"
    v-model:open="show"
    :title="`${formState.id ? '编辑' : '创建'}LINE接入渠道`"
    @cancel="handleCancel"
  >
    <template #footer>
    ...
    <div class="add-wechat-app-alert" v-if="step === 1">
      <a-alert class="zm-alert-info" type="info" show-icon>
        <template #message>
          <div>请按照 <a href="https://help.chatwiki.com/zh/docs/configuring-line" target="_blank">配置步骤</a>配置完成后，在LINE开发者后台获取以下信息。</div>
        </template>
      </a-alert>
      ...
      <div class="alert-body">
        <div class="form-box">
          <a-form ref="formRef" layout="vertical" :model="formState" :rules="formRules">
            ...
            <a-form-item label="LINE ID" name="app_id">
              <a-input v-model:value.trim="formState.app_id" placeholder="请输入LINE ID" />
            </a-form-item>
            <a-form-item label="LINE 口令" name="app_secret">
              <a-input v-model:value.trim="formState.app_secret" placeholder="请输入LINE口令" />
            </a-form-item>
          </a-form>
        </div>
      </div>
...
const defaultAvatar = '/upload/default/line_avatar.png'
